...
- **MetricVariance types:** `autotune_adaptor::MetricVariance`, `autotune_state::MetricVariance`, and `autotune_score::MetricVariance` are three separate (identical) structs — one per leaf crate that needs the per-metric noise estimate (measure / persist / score) — mapped at the `main.rs`/`benchmark` boundaries like the `Direction` enums, so each crate stays a leaf. See [notes/scoring-and-rank.md](notes/scoring-and-rank.md) (noise-aware scoring).
...
4. **Open an issue on [Roger-luo/autotune](https://github.com/Roger-luo/autotune/issues)** for the proper generalized version of any ad-hoc infrastructure you added, and for any follow-up scenario coverage you identified but didn't implement. Use `enhancement` for missing features, `bug` for broken behavior. Link the issue from a code comment if the ad-hoc path is load-bearing for the test.
