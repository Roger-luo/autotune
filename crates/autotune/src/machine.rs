    ...
    loop {
        ...
        match state.current_phase {
            ...
            Phase::Testing => {
                ...
                let test_results =
                    autotune_test::run_all_tests(&config.test, &approach.worktree_path)
                ...
                if all_pass {
                    ...
                    let test_output: String = test_results
                        ...
                        .collect();
                    let _ = store.save_test_output(
                        state.current_iteration,
                        &approach_mut.name,
                        &test_output,
                    );

                    record_discard(&mut state, store, "tests failed")?;
                }
            }
            ...
            Phase::Benchmarking => {
                ...
                let metrics = autotune_benchmark::run_all_benchmarks(
                    &config.benchmark,
                    &approach.worktree_path,
                )
            ...
            Phase::Scoring => {
                ...
                let best_metrics = ledger
                    ...
                    .rev()
                    .find(|r| {
                        r.status == IterationStatus::Kept || r.status == IterationStatus::Baseline
                    })
                    .map(|r| r.metrics.clone())
                ...
                if score_output.decision == "keep" {
                    ...
                    record_discard(&mut state, store, &score_output.reason)?;
                }
            }
            ...
            Phase::Integrating => {
                ...
                let _ =
                    store.save_iteration_metrics(state.current_iteration, &approach.name, &metrics);
