#!/usr/bin/env node
// Convert rustdoc JSON (docs/.rustdoc/*.json) into Markdown API-reference
// pages under src/content/docs/api/. Astro then renders those pages like any
// other doc, so they pick up the site's theme, Shiki highlighting, and TOC.
//
// Run via `pnpm api:build` (parse only) or `pnpm api:gen` (regenerate JSON
// first, then parse). Targets rustdoc JSON format_version 57.
import {
...
const IN_DIR = join(DOCS, '.rustdoc');
const OUT_DIR = join(DOCS, 'src/content/docs/api');
...
function publicMethods(index, implIds) {
  ...
  for (const id of implIds || []) {
    ...
    // Inherent impl: render its public methods.
...
function variantShape(index, v) {
  const kind = v.kind;
  if (kind === 'plain' || kind?.plain !== undefined && kind.plain === null) return '';
  if (kind?.tuple) {
...
function collect(j) {
  const index = j.index;
  const seen = new Set();
  const collected = []; // { item, kind, path: [moduleNames] }
  ...
  function walk(modId, path) {
    ...
    for (const childId of mod.inner.module.items || []) {
      ...
      if (kind === 'module') {
        ...
        // Follow `pub use` re-exports into items defined in this crate.
...
function buildPage(crate, j) {
  const index = j.index;
  const { rootDocs, collected } = collect(j);
  const lines = [];
  lines.push('---');
  lines.push(`title: ${crate}`);
  lines.push(`description: API reference for the ${crate} crate, generated from rustdoc.`);
  lines.push('section: API Reference');
  lines.push(`sidebarLabel: ${crate}`);
  lines.push(`order: ${ORDER[crate] ?? 100}`);
  lines.push('---');
  lines.push('');
  lines.push('<!-- GENERATED by scripts/gen-api.mjs from rustdoc JSON. Do not edit by hand; run `pnpm api:gen`. -->');
  lines.push('');
  lines.push('> Generated from rustdoc JSON. Run `pnpm api:gen` to refresh after changing the crate.');
  lines.push('');
...
if (!existsSync(IN_DIR)) {
  console.error(`No rustdoc JSON found at ${IN_DIR}. Run \`pnpm api:rustdoc\` first.`);
  process.exit(1);
}

mkdirSync(OUT_DIR, { recursive: true });
// Clear previously generated pages so removed crates don't linger.
...
for (const file of files) {
  ...
  const j = JSON.parse(readFileSync(join(IN_DIR, file), 'utf8'));
  if (j.format_version !== 57) {
    console.warn(`! ${crate}: format_version ${j.format_version} (built for 57) — output may be off`);
  }
  const { md, itemCount } = buildPage(crate, j);
  writeFileSync(join(OUT_DIR, crate + '.md'), md);
  console.log(`${crate}: ${itemCount} public items`);
  total += itemCount;
}
console.log(`\nWrote ${files.length} API pages (${total} items) to ${OUT_DIR}`);
